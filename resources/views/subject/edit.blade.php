            ...
            @foreach($subject->teachers()->get() as $teacher)
              <li class="list-group-item">
                ...
                <form class="float-right"
                      action="{{ action('DetachController@detachTeacher', ['subject' => $subject->id, 'teacher' => $teacher->id]) }}" method="post">
                  @csrf
                  <input class="btn btn-danger" type="submit" value="Detach"/>
                </form>
                <a href="{{ url("/teacher/$teacher->id/edit") }}" class="btn btn-secondary float-right">Edit/View</a>
              </li>
            @endforeach
          </ul>
        </div>
